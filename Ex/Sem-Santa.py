        ...
        while not (ready == 2):   
            ...
            if deers_ready:
                ...
                for question in range(WAITING_ROOM_SIZE):
                    logging.debug(f"Helps the elf {question+1}/{WAITING_ROOM_SIZE}")
...
class Elf(threading.Thread):
    ...
    def run(self):
        ...
        for question in range(NUM_QUESTIONS):
            ...
            if elfs_waiting == 3:
                logging.debug(f"I have a question and I'm {elfs_waiting}, SANTAAA !!!! ")
...
class Deer(threading.Thread):
    ...
    def run(self):
        ...
        threading.currentThread().name = type(self).__name__ + "-" + str(int(threading.currentThread().name.split("-")[1])-NUM_ELFS) # Poner nombre al thread
